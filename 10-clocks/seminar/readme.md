...
Когда мы работаем с распределенной системой, мы хотим, чтобы данная система внешне вела себя как один компьютер. Однако внутри себя данная система состоит из нескольких узлов, которые могут изменять общее состояние. Более того, зачастую система позволяет делать конкурентные запросы. 
...
Модель согласованности (consistency model) это контракт между распределенной системой и её клиентом, позволяющий понять, какие гарантии предоставляет система касательно запросов к данным.

В случае отказа сети типа network partition, когда система распадается на изолированные части, появляется неизбежный компромисс между согласованностью и доступностью. Система может оставаться строго согласованной и не возвращать устаревшие значения, но ценой задержки ответов на запросы до восстановления сети. Или же система может продолжать отвечать клиентам как ни в чем не бывало, но не гарантируя актуальность ответов. Это наблюдение получило известность как [CAP теорема](https://en.wikipedia.org/wiki/CAP_theorem).

С точки зрения клиентов логично ожидать от системы, чтобы она вела себя так, будто конкурентных запросов не существует, а их обработка происходит в некотором порядке. В идеале поведение системы должно быть неотличимо от работы с единственной копией данных. Однако чем строже (ближе к идеалу) модель согласованности, тем сложнее этого добиться, что сказывается на ограничениях и скорости работы системы. Компромисс между согласованностью и задержкой в условиях нормальной работы сети описан в [PACELC теореме](https://en.wikipedia.org/wiki/PACELC_design_principle).

![Jepsen consistency models](img/jepsen-consistency.png)

*Левая часть посвящена моделям согласованности транзакций (изменение нескольких ячеек данных), правая &mdash; изменению одного значения*.

С описанием многих моделей согласованности можно ознакомиться на ресурсе [Jepsen](https://jepsen.io/consistency/models) (схема кликабельна), мы затронем лишь несколько из них.

### Causal Consistency

Причинная согласованность требует, чтобы все события с причинно-следственными связями появлялись в соответствующем порядке на всех узлах.

Рассмотрим ситуацию с тремя узлами: узел A отправляет другим узлам сообщение "Поедим?". Узлы B и C отвечают узлу A "да" и "нет" соответственно. Causal consistency позволяет узлам наблюдать сообщения в порядке ("Поедим?", "да", "нет") или ("Поедим?", "нет", "да"). Однако никакой узел не должен наблюдать сообщения "да" и "нет" до сообщения "Поедим?".

Ранее в курсе мы уже встречались с этой моделью, когда рассматривали causal broadcast. Данную модель можно реализовать с помощью векторных часов.

### Sequential Consistency

Leslie Lamport [определяет](https://www.microsoft.com/en-us/research/uploads/prod/2016/12/How-to-Make-a-Multiprocessor-Computer-That-Correctly-Executes-Multiprocess-Programs.pdf) модель последовательной согласованности (sequential consistency) следующим образом:
...
Рассмотрим систему из двух узлов, выполняющих операции над двумя переменными x и y, значения которых изначально равны 0. На представленной схеме описаны несколько возможных результатов исполнения и один невозможный (в рамках SC):

![Sequential Consistency order](img/seq-cons.jpg)

В данной модели процесс может сильно опережать или отставать от других узлов. Из этого следует, что реплика может возвращать информацию произвольной давности. Однако если узел A применил некоторую операцию узла B, он уже не сможет "наблюдать" состояние, актуальное до применения данной операции. Например, прочитав значение x=1 уже нельзя прочитать y=0.

### Linearizability

#### Отступление про истории

Рассмотрим систему, поддерживающую базовый набор операций: запись и чтение. Операции требуют некоторое время на обработку, то есть у каждой операции есть время её начала и время окончания выполнения (когда мы получаем `ok`). Также наша система будет поддерживать возможность конкурентных операций, интервалы выполнения которых пересекаются.

Введем понятие истории (history). Это упорядоченный список записей, где для каждой операции есть запись о её начале и о её окончании.
...
Назовём историю последовательной (sequential history), если после каждого начала операции сразу же следует ответ на данную операцию. Важно учитывать, что по умолчанию, когда говорят про линеаризуемость, имеют ввиду работу с одним объектом.
...
На практике данная модель согласованности запрещает подобные ситуации:
...
B: read
B: ok [1]
# external communication happened here
C: read
C: ok [0] # (!)
```
...
- Операция чтения должна возвращать **наиболее актуальное** записанное значение.
- **Все последующие** операции чтения должны возвращать один и тот же результат до следующей операции записи.

Отличие данных моделей согласованности заключается в определении понятий "наиболее актуальное" и "все последующие".

Для sequential consistency эти понятия определяются в рамках одного клиента. Наиболее актуальное - относительно записей, которые уже видел этот клиент. Последующие чтения - аналогично только в рамках некоторого клиента. Если прочитать значение от лица одного клиента, а потом прочитать это же значение от лица другого клиента, второе чтение может вернуть менее актуальное значение, чем первое чтение.

Для линеаризуемости эти понятия определяются на уровне всех клиентов и в привязке к реальному времени. Мы требуем, чтобы результаты операций даже на разных клиентах были согласованы с реальным временем.
...
Для решения подобных задач можно прибегнуть к подходам автоматического разрешения конфликта. Данные алгоритмы способны работать в системах, где записи генерируются на нескольких узлах, узлы могут уходить в "оффлайн", и необходимо синхронизовать изменения между узлами. При этом требуется обеспечить свойство сходимости (convergence) - все узлы должны прийти к одинаковому состоянию после того, как получат все изменения.

Подход [Operational Transformation](https://en.wikipedia.org/wiki/Operational_transformation) (OT) требует наличия центрального сервера, занимающегося синхронизацией данных. OT способны лучше передавать намерение операций при возникновении конфликтов, но довольны сложны в реализации, поэтому не будут рассмотрены в данном материале. 

State-based CRDT - разновидность структур данных [Conflict-free Replicated Data Types](https://en.wikipedia.org/wiki/Conflict-free_replicated_data_type) (CRDT), которые позволяют обойтись без центрального сервера. Их идея заключается в том, что можно определить удобную операцию *merge* для двух состояний, применяя которую мы будем автоматически разрешать конфликты (детерминированно).
...
Рассмотрим примеры реализации примитивов с помощью данного подхода.

### G-Counter (grow-only counter)

Это простейший монотонно увеличивающийся счетчик. Например, такой примитив подходит для подсчета числа визитов сайта google.com.
...
Чтобы поддержать возможность декремента счетчика, необходимо взять два G-Counter счетчика. В одном будем поддерживать сумму инкрементов, в втором &mdash; декрементов.

Чтобы определить актуальное значение счетчика, необходимо из суммы первого вектора вычесть сумму второго вектора.

### G-Set (grow-only set)

Для реализации структуры типа множество (set), поддерживающей только добавление новых элементов, в качестве *merge* достаточно взять стандартную операцию объединения множеств.
...
С помощью CRDT можно реализовать даже коллаборативный редактор в духе Google Docs. Необходимо представить, что позиции букв в редакторе выражены некоторым рациональным числом, а при добавлении нового символа, ему назначается позиция, равная среднему значению двух соседних символов:
...
Изучить логику работы коллаборативного редактора на основе OT можно [здесь](https://operational-transformation.github.io/).
...
Не будем пересказывать логику реализации других примитивов, так как лучше всего ознакомиться с оригинальными статьями, ссылки на которые есть в конце.
...
- [Timestamps in Message-Passing Systems That Preserve the Partial Ordering](http://cs.engr.uky.edu/~manivann/cs570/Fidge.pdf) &mdash; статья про векторные часы.
- [Jepsen Consistency Models](https://jepsen.io/consistency/models) &mdash; краткое описание моделей согласованности.
...
- [Distributed Systems 8.1: Collaboration software](https://www.youtube.com/watch?v=OqqliwwG0SM) &mdash; Мартин Клеппман доступным языком рассказывает базовые подходы в CRDT и OT.
