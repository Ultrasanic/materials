...
pub fn test_process_join_cannot_receive(config: &TestConfig) -> TestResult {
    let mut rand = Pcg64::seed_from_u64(config.seed);
    let mut sys = build_system(config);
    let mut group = sys.process_names();
    group.shuffle(&mut rand);
    ...
    // process tries to join the system but cannot receive messages
    // it must not be kept in the group
    sys.network().drop_incoming(&new_proc);
    sys.send_local_message(&new_proc, Message::json("JOIN", &JoinMessage { seed }));
    step_until_stabilized(&mut sys, group.into_iter().collect())
}
