...
pub fn test_basic(config: &TestConfig) -> TestResult {
    ...
    println!("Key {key} replicas: {replicas:?}");
    println!("Key {key} non-replicas: {non_replicas:?}");
    ...
    // check the use of class attributes
...
pub fn test_concurrent_get_put(config: &TestConfig) -> TestResult {
    let mut sys = build_system(config);
    let mut rand = Pcg64::seed_from_u64(config.seed);
    ...
    // simultaneously send a PUT request and attempt to GET before the PUT completes
    // on the same non-replica
    send_put(&mut sys, &non_replicas[0], &key, &value, 2);
    // with default seed, the GET will not see the result of PUT
    check_get(&mut sys, &non_replicas[0], &key, 2, None, 10)?;
    // PUT should be completed eventually
    check_put_result(&mut sys, &non_replicas[0], &key, &value, 100)?;
    // the value should be returned by subsequent GET
    check_get(&mut sys, &non_replicas[0], &key, 2, Some(&value), 10)
}
...
pub fn test_concurrent_writes(config: &TestConfig) -> TestResult {
    ...
    // but it was not observed by the put with quorum 2 from the first non-replica!
    check_put_result(&mut sys, &non_replicas[0], &key, &value, 100)?;
