...
pub fn test_basic(config: &TestConfig) -> TestResult {
    ...
    // put key from the first replica (clock skew is 1)
    sys.set_node_clock_skew(&replicas[0], 1.);
    ...
    let skew = -sys.time() + 0.5;
    sys.set_node_clock_skew(&non_replicas[2], skew);
    sys.set_node_clock_skew(&replicas[0], skew);
    sys.set_node_clock_skew(&replicas[1], skew);
    sys.set_node_clock_skew(&replicas[2], skew);
